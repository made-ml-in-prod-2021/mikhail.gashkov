...
from exceptions import NotExistingModelException, ModelNotCreatedException
...
class Classifier:
    ...
    def fit(self, X: np.ndarray, y: np.ndarray) -> None:
        ...
        assert X.shape[0] == y.shape[0], f'Wrong datashapes! X.shape = {X.shape}, y.shape = {y.shape}'
        self.model.fit(X, y)
    ...
    def predict(self, X: np.ndarray) -> np.ndarray:
        if self.model is None:
            raise ModelNotCreatedException
        assert X.shape[1] == self.features_num, f'Model uses {self.features_num} features, your X is {X.shape}'
    ...
    def get_classification_report(self, X: np.ndarray, y_true: np.ndarray) -> Dict[str, dict]:
